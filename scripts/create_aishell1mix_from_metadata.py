...
parser.add_argument('--aishell1mix_outdir', type=str, default=None,
...
def main(args):
    ...
    aishell1mix_outdir = os.path.join(aishell1mix_outdir, f'aishell1_{n_src}mix')
    ...
    create_aishell1mix(aishell1_dir, aishell1mix_outdir, metadata_dir,
                    freqs, n_src, modes, types)


def create_aishell1mix(aishell1_dir, out_dir, metadata_dir,
                    freqs, n_src, modes, types):
    ...
    for md_filename in md_filename_list:
        ...
        process_metadata_file(csv_path, freqs, n_src, aishell1_dir, out_dir, modes, types)


def process_metadata_file(csv_path, freqs, n_src, aishell1_dir, out_dir, modes, types):
    ...
    for freq in freqs:
        ...
        for mode in modes:
            ...
            dir_name = os.path.basename(csv_path).replace(
                f'aishell1_{n_src}mix_', '').replace('-clean', '').replace(
                '.csv', '')
            ...
            process_utterances(md_file, aishell1_dir, freq, mode,
                               subdirs, dir_path, subset_metadata_path, n_src)


def process_utterances(md_file, aishell1_dir, freq, mode, subdirs,
    ...
    for results in tqdm.contrib.concurrent.process_map(
        functools.partial(
            process_utterance, 
            n_src, aishell1_dir, freq, mode, subdirs, dir_path),
        ...
        for mix_id, snr_list, abs_mix_path, abs_source_path_list, length, subdir in results:
            ...
            add_to_mixture_metadata(md_dic[f'mixture_{dir_name}_{subdir}'],
                                    mix_id, abs_mix_path, abs_source_path_list, length, subdir)
...
def process_utterance(n_src, aishell1_dir, freq, mode, subdirs, dir_path, row):
    res = []
    # Get sources and mixture infos
    mix_id, gain_list, sources = read_sources(row, n_src, aishell1_dir)
    # Transform sources
...
def read_sources(row, n_src, aishell1_dir):
    ...
    for sources_path in sources_path_list:
        sources_path = os.path.join(aishell1_dir,
                                    sources_path)
        source, sample_rate = sf.read(sources_path, dtype='float32', samplerate=None)
        assert sample_rate==RATE, 'The audio samplerate should be 16kHz!'
        # Get max_length
...
def add_to_mixture_metadata(mix_df, mix_id, abs_mix_path, abs_sources_path,length, subdir):
    """ Add a new line to mixture_df """
    sources_path = abs_sources_path
    if subdir == 'mix_single':
